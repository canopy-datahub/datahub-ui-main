...
// New function for creating new study data points (no existing study to compare against)
export function addNewStudyDataPoint(dirtyField, studyUpdate, formFields, entityProperties) {
    // For new studies, we simply add all dirty fields as new data points
    const value = formFields[dirtyField];
    // Never emit a null/undefined/empty value: the backend trims property values and
    // a null would blow up study creation. A missing value means "no value" -> omit it.
...
// New function for processing array data for new studies
export function processNewStudyArrayData(dirtyField, array, studyUpdate, formFields, entityProperties) {
    // Get the entityID for this field
    let entityID;
    for (let k = 0; k < entityProperties.length; k++) {
        if (entityProperties[k].name === dirtyField) {
            entityID = entityProperties[k].id;
    ...
    // Add all array values as new data points
    for (let j = 0; j < array.length; j++) {
        if (array[j] !== null && array[j] !== undefined && array[j] !== '') {
            const value = typeof formFields[dirtyField][j] === 'string' ? formFields[dirtyField][j] : formFields[dirtyField][j]?.value;
            // A non-string element without a `.value` (e.g. a malformed option object)
            // yields undefined here; skip it so we never send a null property value.
            if (value === null || value === undefined || value === '') {
                continue;
            }
            studyUpdate.studyPropertyValues.push({
