...
const useRest = () => {
    ...
    const makeCall = async (type, path, body, options) => {
        let callResults = {};
        // Refresh the access token if it's within 30s of expiring. This is a local
        // exp-claim check in the common case (no network call) and prevents requests
        // from going out with a token that's about to be — or just was — rejected as expired.
        if (keycloak) {
            try {
                await keycloak.updateToken(30);
            } catch {
                // Refresh failed (refresh token expired, network blip, etc.) — fall through
                // and let the request fail; the 401 path below will surface it.
            }
        }
        // Always read the token from the live Keycloak instance so that
        // auto-refreshed tokens are used even if the React state hasn't updated.
        const currentToken = keycloak?.token || token;
        // Base headers
        const baseHeaders = options?.headers
            ? { ...options.headers, 'Cache-Control': 'no-cache', 'X-Content-Type-Options': 'nosniff' }
            : {
                'Content-Type': 'application/json',
                'Cache-Control': 'no-cache',
                'X-Content-Type-Options': 'nosniff',
            };
        // Add Keycloak JWT token if available
        ...
        try {
            ...
            const tempNotification = { ...BaseNotification };
            // Prefer the backend's own message so business-rule errors (e.g. "Data Submitter
            // requires a center") are surfaced verbatim. Fall back to the per-call
            // `options.errorMessage` for generic network/transport failures.
            tempNotification.message = e?.response?.data.message || options.errorMessage || e?.message || 'Error making call.';
