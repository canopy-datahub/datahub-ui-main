...
**Canopy** is an open-source platform for FAIR-aligned scientific data hubs, derived from the NIH RADx Data Hub. It enables data sharing, harmonization, discovery, and reuse across research studies. The frontend is built with **React** and **Next.js** for server-side rendering, routing, and optimized performance.
...
-   **User Registration & Authentication:** To gain access to specific features, a user can register and authenticate using Keycloak as the Identity Provider (IdP).
-   **Approved Data:** Authenticated users can access data files based on their data access approvals.
...
Early components were developed within [Storybook](https://storybook.js.org/docs/react/get-started/install/), a UI sandbox where components can be isolated, making it easier to build, document, and visually test components outside of the main application. For more information, [see below](#storybook).
...
-   `/pages/postAuth`: When a user logs in via Keycloak, they are directed back to the application via this url path.
...
Authentication is required to access specific features. Canopy uses [Keycloak](https://www.keycloak.org/) as its Identity Provider (IdP) for authentication and authorization. When a user logs in via Keycloak, they are redirected to `/postAuth` with a session ID that is used to retrieve user information from the backend services.

#### Login Locally

1. Ensure that a Keycloak account has been created in your DEV environment's Keycloak realm.
2. Sign into a DEV environment using Keycloak.
3. Inspect the page and note the Cookie `chocolateChip` value.
4. For your local environment, there are 2 ways to use the DEV session:
    ...
    - Recreate the `chocolateChip` cookie on the local browser and refresh the page.
